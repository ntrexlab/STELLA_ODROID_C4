    ...
    while (run)
    {
        if (MW_SerialRecv(&id, &length, data))
        {
            ...
            case DEG:
                ...
                imu.orientation.w = 0; //(COS(x) * COS(y) * COS(z)) + (SIN(x) * SIN(y) * SIN(z));
                imu.orientation.x = x; //(COS(x) * COS(y) * SIN(z)) - (SIN(x) * SIN(y) * COS(z));
                imu.orientation.y = y; //(COS(x) * SIN(y) * COS(z)) + (SIN(x) * COS(y) * SIN(z));
                imu.orientation.z = z; //(SIN(x) * COS(y) * COS(z)) - (COS(x) * SIN(y) * SIN(z));
                break;
            }
        }
    }
